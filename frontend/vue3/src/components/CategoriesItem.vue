<template>
    <tr>
        ...
        <td><span
                v-bind:class="[categoryitem.status === 'disable' ? 'badge bg-danger p-2 m-1' : 'badge bg-success p-2 m-1']"
                @click.stop="viewCategory(categoryitem.id)"> {{ categoryitem.status }} </span></td>
        <td>
            ...
            <!-- <span class="badge bg-warning  p-2 m-1 text-dark"
                @click.stop="updateStatus(categoryitem.id, categoryitem.status)"> Edit status </span> -->
